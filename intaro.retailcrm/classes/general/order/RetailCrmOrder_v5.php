...
class RetailCrmOrder
{
     ...
     */
    public static function orderSend(array $arFields, $api, $arParams, bool $send = false, $site = null, string $methodApi = 'ordersEdit')
    {
    ...
    public static function uploadOrders(int $pSize = 50, bool $failed = false, array $orderList = []): bool
    {
        ...
        if (count($resOrders) > 0) {
            if (false === self::uploadCustomersList($resCustomers, $api, $arParams['optionsSitesList'])) {
                return false;
            }
            ...
            if (false === self::uploadOrdersList($resOrders, $api, $arParams['optionsSitesList'])) {
                return false;
            }

            if ($failed == true && $failedIds !== false && count($failedIds) > 0) {
                RetailcrmConfigProvider::setFailedOrdersIds(array_diff($failedIds, $recOrders));
            } elseif ($lastUpOrderId < max($recOrders) && count($orderList) === 0) {
     ...
     * @throws \Bitrix\Main\SystemException
     */
    public static function orderObjToArr($obOrder): array
    {
